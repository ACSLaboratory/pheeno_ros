...
float northOffset;//Offset such that whatever orientation the robot is placed down
                  //it will be oriented along the global x-axis.
...
void loop() {
  while (calculateDistance(myRobot.botXPos,myRobot.botYPos,botXf[count%numWayPoints],botYf[count%numWayPoints]) > 3){
    ...
    myRobot.sensorFusionPositionUpdate(northOffset); //Encoders used for state estimates.
  }
  count ++;
}
